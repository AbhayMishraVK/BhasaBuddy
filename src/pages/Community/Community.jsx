              ...
              posts.map((post,index) => (
                <div className={styles["comm-post"]} key={post.id}> {/* Apply CSS module class */}
                  ...
                  <div className={styles["comm-lower"]}> {/* Apply CSS module class */}
                    <button onClick={() => handleLike(post.post_id,index) } disabled={disabledButtons.includes(index)} className={styles["comm-button"]}>Like<span style={{ marginLeft: '5px' }}>{post.likes}</span></button>
                    <button onClick={() => handleDislike(post.post_id,index)} disabled={disabledButtons.includes(index)} className={styles["comm-button"]}>Dislike<span style={{ marginLeft: '5px' }}>{post.dislikes}</span></button>
                  </div>
                </div>
              ))
            )}
